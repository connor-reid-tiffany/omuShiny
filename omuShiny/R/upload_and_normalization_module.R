upload_and_normalization_UI <- function(id) {
  ...
  tagList(
    ...
    fileInput(inputId = ns("upload_metabo"), "Upload Metabolomics Data", accept = c(".csv")),
    fileInput(inputId = ns("upload_meta"), "Upload Sample Data", accept = c(".csv")),
    ...
    selectizeInput(inputId = ns('histo_metabolite'), "Pick Metabolite", choices = NULL, multiple = TRUE),
    selectizeInput(inputId = ns('histo_group'), 'Pick Variable to Group By', choices = NULL, multiple = FALSE),
    numericInput(inputId = ns("bins"), label = "Adjust Number of Bins", value = 15),
